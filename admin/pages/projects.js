  ...
  window.__pages['projects'] = {
    ...
    init: async function () {
        ...
        ensureEngineers().then(()=>{
          ...
          wrap.querySelectorAll('input[type=checkbox][data-eng]').forEach(cb=>{
            cb.addEventListener('change',()=>{
              ...
              lbl.style.background  = cb.checked?'rgba(78,141,245,.18)':'rgba(var(--fg-rgb), .04)';
      ...
      async function openAssignEngineers(projectId, projectName){
        try {
          ...
          openModal(`تخصيص مهندسين — ${esc(projectName)}`,
            `<div class="modal-body">
              <div style="margin-bottom:12px;padding:10px 14px;background:rgba(78,141,245,.08);border:1px solid rgba(78,141,245,.2);border-radius:10px;font-size:.82rem;color:var(--accent);display:flex;align-items:center;gap:8px">
      ...
      function _warrantyHtml(saleDate) {
        ...
        return `
          <div style="margin-top:16px;padding:14px;background:rgba(78,141,245,.05);border:1px solid rgba(78,141,245,.2);border-radius:12px">
            <div style="font-size:.78rem;font-weight:800;color:#4e8df5;margin-bottom:11px;text-transform:uppercase;letter-spacing:.5px"><i class="ri-shield-check-line"></i> حالة الضمانات</div>
            ${items.map(w => {
              ...
              return `
                <div style="padding:8px 0;border-bottom:1px dashed rgba(var(--fg-rgb), .06)">
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;gap:8px">
                    <span style="color:var(--light);font-size:.84rem;font-weight:700">${w.ar} <small style="color:#8fa3c0">(${w.years} سنين)</small></span>
